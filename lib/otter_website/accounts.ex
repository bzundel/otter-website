defmodule OtterWebsite.Accounts do
  @moduledoc """
  ...
  alias OtterWebsite.Accounts.{User, UserToken, UserNotifier}
  ...
  def mark_invitation_key_used(key, user) do
    case Repo.one(from ik in InvitationKey, where: ik.key == ^key and is_nil(ik.used_by)) do
      nil -> throw "Invitation key to be marked as used cannot be found"
      invitation_key ->
        invitation_key
        |> Ecto.Changeset.change(used_by: user.email)
        |> Repo.update()
    end
  end
end
